...
const TableList = ({ onShowDetail, onDeleteCategory }) => {
	...
	useEffect(() => {
		dispatch(fetchCategoryList("NEWS"));
	}, [dispatch]);
	...
	const columnsCheck = useCanMultiple(["news-menu.edit", "news-menu.delete"]) ? columns : columns.filter((q,index) => index != columns.length - 1);
